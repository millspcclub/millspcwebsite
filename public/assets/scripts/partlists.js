    ...
    fetch("/data/parts")
        ...
        .then(data => {
            for (const partlist of data.lists) {
                var card = $("<card class='small'></card.text>")
                    .append(icons[partlist.type])
                    .append(`<h3>${partlist.name}</h3>`)
                    .append(`<div class="price">$${partlist.price}</div>`)

                const link = $(`<a style="color: inherit" href="https://pcpartpicker.com${partlist.relURL}"></a>`)
                    .append(card);
